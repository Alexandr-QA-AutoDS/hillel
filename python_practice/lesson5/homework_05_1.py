...
filtered = [(name, data) for name, data in car_data.items() if
            data[1] >= year and data[2] >= engine and data[4] <= price]

filtered.sort(key=lambda x: x[1][4])

for name, data in filtered[:5]:
    print(f"{name}: {data}")
